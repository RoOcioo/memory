...
class Board extends React.Component {
  ...
  flip(cardIndex) {
    if (this.state.firstCard === null) { // au depart de la valeur du flip est NULL
      this.setState({ firstCard: cardIndex }); // initialise la valeur de la 1ere carte par le 1er flip
      // c'est seulement après ce premier flip qu'on entre dans le jeu car firstCard aura une valeur
    } else {
      ...
      if (firstCardContent === secondCardContent) {
        this.setState({ firstCard: null }); // on a un match, du coup on remet la valeur du flip à Null
      } else {
        setTimeout(() => {
        ...
        }, 1500) // definit le temps d'attente après un mauvais match, puis remets à Null le flip
      }
    }
